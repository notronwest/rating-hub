...
- **Carry-forward active priorities across sessions.** Today, `generate-priorities` only protects rows with `pinned = true OR edited = true` — both signals scoped to the current session. When a player records a *new* session, their previously *active* priorities from prior sessions don't carry into the new session report; the new session generates a fresh draft set in isolation. Long-term, the player's "Working on" view aggregates active priorities cross-session, but the new session report should also inherit / reference them: ideas worth pursuing — (a) when generating priorities for session N, pass session N-1's still-active priorities into the protected set so the model doesn't duplicate them, and shadow-display them as "still working on" in a sub-section; (b) auto-recompute `lead_stat_value_latest` on the carried-forward priorities so the player sees their delta. Either approach preserves the existing per-session generate while honoring continuity. Touches `generate-priorities` (cross-session protected set), `PrioritiesPanel` (a "Still working on (from previous sessions)" subsection above the new drafts), `v_player_active_priorities` (already exists from migration 025).

- **Player Working-on view (cross-session).** Migration 025 added `v_player_active_priorities` aggregating active priorities + strengths across sessions, but no UI consumes it yet. Build a "Working on" tab on the player profile (`/players/:slug` — the rating-report page or its successor) showing the cross-session active list, recent stat deltas, and a small "Mastered" archive button per row. Also surfaces the lead-stat trend (`lead_stat_value_at_creation` vs `lead_stat_value_latest`) per priority — needs a backfill job to update `lead_stat_value_latest` whenever a new game lands.

- **Cross-session patterns (the real "common themes").** The original CommonThemesPanel was per-session AI-generated takeaways — fully redundant with priorities + strengths and now removed from the session report. The genuinely different feature: surfacing patterns that recur across MULTIPLE sessions (e.g. "Drives the 3rd shot whenever opponent serves to the backhand · 4 sessions in a row · still missing 60%+"). Different scope from priorities (one session) and from working-on view (active priorities the coach explicitly chose). Approach: a new `player_coaching_patterns` table or repurpose `kind='theme'` rows to be session-aggregated. A new edge function `generate-cross-session-patterns` runs over the player's last N sessions of stats + priorities + flagged moments and emits patterns with: title, summary, sessions_observed (array of session_ids), trajectory ("worsening / stable / improving"), and a "still relevant" check that runs each session. Surfaces on the player profile, NOT the session report (it's not session-bound). Existing `generate-themes` edge function and the `kind='theme'` rows in `player_coaching_themes` can be deleted or repurposed when this lands — they're currently dead code paths. CommonThemesPanel.tsx component file also dead-code at this point.
